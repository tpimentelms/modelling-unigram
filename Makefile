...
run_two_stage_type_training: $(CHECKPOINT_TYPE_FILE)
	...
	mkdir -p $(RESULTS_DIR_LANG)
	python src/h02_learn/train_two_stage.py --data-file $(PROCESSED_DATA_FILE) --generator-path $(CHECKPOINT_TYPE_PATH) \
			--alpha $(ALPHA) --beta $(BETA) --two-stage-state-folder $(TWO_STAGE_INIT_TYPE_STATE_FOLDER) --epochs $(EPOCHS) \
			--adaptor-results-file $(TWO_STAGE_TYPE_TRAINING_RESULTS_FILE)
...
$(CHECKPOINT_TYPE_FILE): $(PROCESSED_DATA_FILE)
	...
	python src/h02_learn/train_generator.py --data-file $(PROCESSED_DATA_FILE) --generator-path $(CHECKPOINT_TYPE_PATH) --dataset types
